...
def generateBenchmarkReport(logFile,envLog,filterList,tempDir,outFile="",coverage='a',verbose=False):
    ob=benchmark(logFile,envLog,outDir=tempDir)
    #generate benchmarks
    ob.plot_time_perobject()
    ob.ploplot_time_perprogram()
...
def benchmark():
    ...
    generateBenchmarkReport(logFile,envLog,filters,tempDir,outFile=outFile,coverage=args.c,verbose=args.v)
