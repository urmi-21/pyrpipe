...
class Samtools(RNASeqTools):
    ...
    More specific parameters can be passed when calling the specific functions. There is option to overside the parameters specified here in init later.
    
    threads: int
        Number of threads samtools will use
    out_dir: string
        Path where all output will be generated by dfefault
    max_memory: int
        Max memory to use in GB
    
    """
    def __init__(self,threads=None,out_dir=None,max_memory=None):
        ...
        #if out_dir doesnt exist then create
        if self.out_dir:
            if not pu.check_paths_exist(out_dir):
                pu.mkdir(self.out_dir)
